...
Note: 简单随机抽样是不放回的抽样，不是二项分布设定，因为每取出一个, 概率P就，改变了；但是如果总体size远大于样本size，那么
放回抽样和不放回抽样就是大致一致的，服从于二项分布，服从于正态曲线。
...
form(points cluster around a line or other), strength(how closethe points follow the form)
...
expected value and SE of the this statistic, computed under the assumption H0 is true.
