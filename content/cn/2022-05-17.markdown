...
> $$ P(X = k) = \frac{C_M^K C_{N-M}{n-k}}{C_N^n} $$  \\( E(X) = n\frac{M}{N} \\), \\( D(X) = \frac{nM(N-M)(N-N)}{N^2(N-1)} \\)

**泊松分布(Possion Distribution)**：涉及到单位时间，面积，体积的计数过程，数量X:
> $$ P(X=k) = \frac{\lambda^k e^{-\lambda}}{k!}  $$  \\( E(X) = \lambda), D(X) = \lambda \\)
